...
보통은 자동으로 업데이트로 설정하는데 나는 아무런 업데이트를 하지 않는 아주 훌륭한 유저이다. 그 버그가 왜 생겼는지 문제가 지속적으로 발생하는 컴퓨터를 가지고 있다는 것은 아주 재밌는 실험환경을 가지고 있다는 것이다.

플러그 앤 플레이(Plug & Play) : 보통 마우스 선을 USB에 꼿아서 쓰는데 이게 가능한 이유가 이러한 기능을 지원해줘서이다. USB도 꼿으면 자동으로 실행되는데 이러한 점 때문에 군대 컴퓨터에 USB 꼿으면 좆되는 것이다. 물론 지금은 휴대폰 써서 그럴 일이 거의 없겠지만...
